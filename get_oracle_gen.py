...
## Taken from https://github.com/clab/rnng/blob/master/get_oracle_gen.py
## Adapted for python 3 + smaller dataset (collapsed all UNK tags to just UNK)
...
def unkify(tokens, words_dict):
    ...
    for token in tokens:
        ...
        if len(token.rstrip()) == 0:
            ...
            ch0 = token.rstrip()[0]
            """
            # for training on 10% PTB, simplify all UNK just to UNK
            if ch0.isupper():
                if numCaps == 1:
                    result = result + '-INITC'    
...
def main():
    ...
    for line in dev_lines:
        ...
        try: 
            ...
            warn('Could not get actions for line # ' + str(line_ctr))
            next
